...
const mouseMove = (el, state, cardInner, cardBackground) => {
  ...
  const angleX = (state.mouseX / state.cardWidth) * 30;
  const angleY = (state.mouseY / state.cardHeight) * 30;

  // Menghitung perpindahan posisi (translate) berdasarkan posisi mouse
  const posX = (state.mouseX / state.cardWidth) * 60;
  const posY = (state.mouseY / state.cardHeight) * 20;
...
popularLists.forEach((card) => {
  ...
  card.addEventListener("mousemove", (e) =>
    mouseMove(e, state, cardInner, cardBackground)
  );
